// ═══════════════════════════════════════════
//  Springer (LNCS) — Single-Column Template
// ═══════════════════════════════════════════

#let render(data) = {
  set document(title: data.title, author: data.authors.map(a => a.name))
  set page(paper: "a4", margin: (top: 1.25in, bottom: 1.25in, left: 1.2in, right: 1.2in))
  set text(font: "Times New Roman", size: 10pt)
  set par(justify: true, leading: 0.55em)
  set heading(numbering: "1.1.1")
  set math.equation(numbering: "(1)")

  // Springer LNCS heading styles
  ...
  align(center)[
    #block(text(weight: "bold", size: 14pt, eval(data.title, mode: "markup")))
    #v(1.5em)
    #grid(
      ...
      ..data.authors.map(a => align(center)[
        ...
        #text(size: 9pt, style: "italic")[#eval(a.affiliation, mode: "markup")]
      ])
    )
  ]
  v(2em)
  ...
  if "references" in data and data.references.len() > 0 {
    ...
    for ref in data.references {
      block(spacing: 0.5em)[
        #str(i). #eval(ref.citation, mode: "markup")
      ]
      i += 1
    }
  }
